                        ...
                        <div class="w-full">
                            ...
                            <form method="POST" action="{{ route('reservations.store.step.two') }}">
                                    ...
                                    <div class="mt-1">
                                        <select id="table_id" name="table_id" class="form-multiselect block w-full mt-1">
                                            ...
                                            <option value="{{ $table->id }}" @selected($table->id == $reservation->table_id)>
                                                {{ $table->name }}
                                                ({{ $table->guest_num }} Guests)
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <span class="text-xs">This is the available tables that can hold {{$reservation->guest_num}} Guests
                                    @error('table_id')
                                ...
                                <div class="mt-6 p-4 flex justify-between">
                                    ...
                                    <button type="submit" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Make
                                        Reservation</button>
                                </div>
                            </form>
                        </div>
